...
async function _safeCreateCast(btClient, castedUuIdentity, code) {
    try {
        await btClient.post("uuUnitGroup/addCast", { code, sideBCode: castedUuIdentity });
    } catch (e) {
...
async function main() {
    ...
    await _safeCreateCast(btClient, uuIdentity,"LOCAL_DEV_BT-authorities");
    await _safeCreateCast(btClient, uuIdentity,"LOCAL_DEV_BT-bt-unit-authorities");
